...
def test_capability_ledger_roundtrip_and_delta(tmp_path: Path) -> None:
    ...
    assert restored.baseline()["regression_count"] == 0


def test_capability_ledger_records_regressions(tmp_path: Path) -> None:
    path = tmp_path / "capability.json"
    ledger = CapabilityLedger(path)
    baseline = CapabilitySnapshot(
        ...
        "base",
        "2026-08-24T00:00:00Z",
        (CapabilityScore("coding", 0.90),),
    )
    candidate = CapabilitySnapshot(
        "v9.1",
        "head",
        "2026-08-24T00:01:00Z",
        (CapabilityScore("coding", 0.80),),
    )

    ledger.record(baseline)
    recorded = ledger.record(candidate)

    assert recorded.regression_count == 1
    assert ledger.baseline()["regressions"] == ["coding"]
